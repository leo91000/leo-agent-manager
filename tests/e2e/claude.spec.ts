...
test('switching coding agents preserves one chat, context and provider selection after reload', async ({ page, workspace }, testInfo) => {
  ...
  await page.getByRole('combobox', { name: 'Coding agent', exact: true }).click()
  await page.getByRole('option', { name: 'Claude Code', exact: true }).click()
  await expect(page.getByText('The next message starts a new agent session', { exact: false })).toBeVisible()
  await page.getByRole('combobox', { name: 'Model', exact: true }).click()
  await page.getByRole('option', { name: 'Sonnet', exact: true }).click()
  for (const width of [1440, 390, 320]) {
  ...
  await page.reload()
  await page.getByRole('button', { name: 'Message options', exact: true }).click()
  await expect(page.getByRole('combobox', { name: 'Coding agent', exact: true })).toHaveValue('Claude Code')
  await page.getByRole('combobox', { name: 'Coding agent', exact: true }).click()
