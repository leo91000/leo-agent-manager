...
test('accounts show live usage, reset windows and accessible controls across themes and mobile sizes', async ({ page, workspace }, testInfo) => {
  ...
  await dialog.getByRole('spinbutton', { name: /Parallel runs/ }).fill('12')
  ...
  await expect(page.getByRole('article', { name: 'Work subscription', exact: true }).getByText('0 / 12 parallel runs')).toBeVisible()
