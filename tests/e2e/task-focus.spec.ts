...
test('compact task workspace keeps the conversation visible and details reachable', async ({ page, workspace }) => {
  ...
  await expectSingleScroll(page)
  await expect(page.getByRole('button', { name: 'Choose task' })).toBeVisible()
  await page.getByRole('button', { name: 'Details', exact: true }).click()
  await expect(page.getByRole('dialog', { name: 'Task details' })).toContainText('0 9 * * 4')
  ...
  await page.getByRole('textbox', { name: 'Search tasks' }).fill('Graphile')
  await page.getByRole('button', { name: new RegExp(task.name) }).click()
  await expect(page.getByRole('button', { name: 'Choose task' })).toHaveAttribute('aria-expanded', 'false')
  await expect(page.getByRole('region', { name: 'Activity output' })).toBeVisible()
