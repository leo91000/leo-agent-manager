...
<template>
  <span class="agent-avatar relative inline-grid shrink-0 place-items-center" :style="{ 'width': `${size}px`, 'height': `${size}px`, '--avatar-radius': `${radius}px` }" aria-hidden="true">
    ...
    <span class="grid size-full select-none place-items-center font-heading font-bold text-white" :style="{ background: identityColor(identity || name), borderRadius: `${radius}px`, fontSize: `${Math.round(size * 0.42)}px` }">{{ initial(name) }}</span>
    <span
      v-if="badge"
