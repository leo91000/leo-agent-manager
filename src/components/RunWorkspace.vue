...
<template>
  <RouterLink v-if="!embedded" to="/runs" class="back-link">
    ...
    <div class="page-heading">
      <div>
        ...
        <div class="run-title-meta">
          <Status :status="run.status" /><span>{{ run.snapshot.agent.name }} · {{ date(run.createdAt) }}</span><button v-if="embedded && active" class="button danger-outline" @click="confirm = true">
            <Square :size="13" />Stop run
          </button>
        </div>
      </div>
      <button
        v-if="active && !embedded"
        class="button danger-outline"
        @click="confirm = true"
      >
        <Square :size="15" />Stop run
      </button><button v-else-if="!embedded" class="button" @click="retry">
        <RotateCw :size="16" />Run again
      </button>
    </div>
    <div v-if="!embedded" class="run-facts">
