...
const chatStatus = computed(() => {
  ...
  if (active.value)
    return detail.value?.run?.status === 'queued' ? 'Waiting' : 'Working'

  if (detail.value?.run?.status === 'failed')
    return 'Failed'

  if (detail.value?.run?.status === 'interrupted')
    return 'Interrupted'
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <section class="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden" aria-label="Chat workspace">
          ...
          <div v-else-if="inactive" class="m-auto w-full max-w-xl px-6 py-12 text-center">
            ...
            <UiButton
              v-if="detail?.lifecycle === 'trash' || detail?.lifecycle === 'archived' || (detail?.lifecycle === 'restoring' && detail.lifecycleError)"
              :disabled="busy"
              variant="primary"
          ...
          <ActivityFeed
            ...
            :cache-key="`/chats/${route.params.id}/stream`"
          ...
          <div v-if="!inactive" class="mx-auto w-full max-w-205 shrink-0 px-5 pb-1 pt-3 phone:px-0 phone:pt-2">
              ...
              <ul v-if="queueOpen" class="max-h-32 overflow-auto border-t border-line px-3 py-1">
                <li v-for="message in pending" :key="message.id" class="flex items-center gap-1.5 py-1.5 text-xs">
                  ...
                  <template v-if="message.status === 'queued'">
                    <button
                      v-if="active && message.mode !== 'steer'"
                      :class="iconButton"
                      aria-label="Steer with this message"
                      title="Steer now"
                      @click="update(message, 'steer')"
                    >
            ...
            <form
              class="chat-composer relative rounded-2xl border border-line/60 bg-raised p-4 shadow-[0_4px_24px_#00000006] focus-within:border-accent/50 phone:p-3"
              @submit.prevent="send()"
