...
The worker reads usage every minute through the official Codex app-server. The page fetches those cached readings every five seconds and shows the short and weekly windows, reset times, low capacity below 5%, and the active run. Stale readings are marked unavailable rather than presented as current. The overall remaining percentage is the lowest remaining window, including applicable model-specific limits when scheduling. “Next run” on the page refers to Codex's general capacity; an agent's model-specific limit can change its selection.
...
Usage-window resets happen on OpenAI's side. The worker observes them automatically; it does not consume earned-reset credits, buy capacity, or change a subscription. Crossing below 5% highlights low usage but does not interrupt a healthy run. A reset timestamp alone never makes an account available: fresh usage must confirm usable capacity.
...
Requires a Codex CLI with `account/read`, `account/rateLimits/read`, and `codex exec resume`. The integration was checked against installed Codex 0.154.0, including a read-only live account/usage query. Session handoff tests use synthetic accounts and a fixture subprocess, not paid production tasks.
