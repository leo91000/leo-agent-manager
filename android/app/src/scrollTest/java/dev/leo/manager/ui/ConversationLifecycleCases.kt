...
abstract class ConversationLifecycleCases {
    ...
    @Test
    fun deletingWorkingConversationFromFilRequiresConfirmation() = withConversation(true, true)
        ...
        MockWebServer().use { server ->
            ...
            val chat =
                Chat(
                    ...
                    status = if (working) "running" else "succeeded",
                )
            server.dispatcher =
                object : Dispatcher() {
                        ...
                        if (path == "/api/chats/chat/stream") {
                            val current =
                                chat.copy(
                                    ...
                                    purgeAt =
                                        if (deleted.get()) System.currentTimeMillis() + 86400000
                                        else null,
                                )
                            val frame =
                            ...
                            when {
                                ...
                                path == "/api/chats/chat" && request.method == "DELETE" -> {
                                    ...
                                    if (
                                        ...
                                        return MockResponse()
                                            .setResponseCode(409)
                                            .setBody("""{"error":"Confirmation requise"}""")
                                    deleted.set(true)
                                    "{}"
                                }
            ...
            compose.setContent {
                ...
                LeoTheme {
                    if (fromFil) LeoApp(vm = vm)
                        ...
                        if (selected == null)
                            ChatsScreen(vm, state, open = { selected = it }, create = {})
                        else
                            ChatScreen(
                                vm,
                                state,
            ...
            row.performTouchInput {
                swipe(Offset(width * .9f, centerY), Offset(width * .7f, centerY), 800)
            }
            compose.waitForIdle()
            ...
            if (working) {
                ...
                compose.waitUntil(10000) {
                    compose
                        .onAllNodesWithText("Arrêter et supprimer ?")
                        .fetchSemanticsNodes()
                        .isNotEmpty()
                }
                ...
                compose.waitUntil(10000) {
                    compose.onAllNodesWithText("Confirmer").fetchSemanticsNodes().isNotEmpty()
                }
                ...
                compose.waitUntil(10000) {
                    compose
                        .onAllNodesWithText("Conversation supprimée")
                        .fetchSemanticsNodes()
                        .isNotEmpty()
                }
            ...
            compose.waitUntil(15000) {
                compose.onAllNodesWithText(chat.title).fetchSemanticsNodes().isEmpty()
            }
            if (fromFil) {
            ...
            compose.waitUntil(15000) {
                compose
                    .onAllNodesWithText("Restaurer la conversation")
                    .fetchSemanticsNodes()
                    .isNotEmpty()
            }
            ...
            compose.waitUntil(15000) {
                compose
                    .onAllNodesWithText("Cette conversation est dans la corbeille.")
                    .fetchSemanticsNodes()
                    .isEmpty()
            }
