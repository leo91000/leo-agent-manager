...
class SkillMentionLogicTest {
    ...
    @Test fun `chat skills follow agent access and the chat project`() {
        val skills = listOf(
            Skill("review"),
        ...
        assertEquals(listOf("$project/deploy", "$project/review"), chatSkills(skills, Agent(), project).map { "${it.scope}/${it.name}" })
        assertEquals(listOf("deploy", "review", "secret"), chatSkills(skills, Agent(), null).map { it.name })
        val limited = Agent(access = AccessPolicy(projects = listOf(project), skills = listOf("global/review", "$other/secret")))
        assertEquals(listOf("global/review"), chatSkills(skills, limited, "").map { "${it.scope}/${it.name}" })
        assertTrue(chatSkills(skills, null, null).isEmpty())
    }

    @Test fun `mentions are detected at the caret like the web composer`() {
    ...
    @Test fun `matches are ranked and insertion replaces the token`() {
        val skills = listOf("code-review", "review", "reviewer", "preview-docs", "ship").map {
            SkillOption(it, if (it == "ship") "Release to production" else "", "global")
        }
        assertEquals(listOf("review", "reviewer", "code-review", "preview-docs"), matchSkills(skills, "review").map { it.name })
        ...
        assertEquals(listOf("code-review", "preview-docs", "review", "reviewer", "ship"), matchSkills(skills, "").map { it.name })
        assertEquals(
            TextFieldValue("Use \$review ", TextRange(12)),
            insertSkill(TextFieldValue("Use \$rev", TextRange(8)), SkillMention(4, 8, "rev"), "review"),
        )
        assertEquals(
            TextFieldValue("Use \$review now", TextRange(12)),
            insertSkill(TextFieldValue("Use \$re now", TextRange(7)), SkillMention(4, 7, "re"), "review"),
        )
    }

    @Test fun `only known skills outside code are highlighted`() {
