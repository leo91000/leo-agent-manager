...
class CodexAccountsJourneyTest {
    ...
    @Test fun `account concurrency above four is saved and restored while invalid input cannot save`() {
        MockWebServer().use { server ->
            ...
            server.dispatcher = object : Dispatcher() {
                override fun dispatch(request: RecordedRequest): MockResponse {
                    val result = when (request.path) {
                        ...
                        "/api/codex/accounts" -> """[{"id":"account","name":"Compte principal","state":"ready","maxConcurrentRuns":${updates.lastOrNull()?.get("maxConcurrentRuns") ?: 4}}]"""
                        "/api/codex/accounts/login" -> "null"
                        "/api/codex/accounts/account" -> {
                            assertEquals("PUT", request.method)
                            assertEquals("fixture", request.getHeader("X-CSRF-Token"))
                            updates += wireJson.parseToJsonElement(request.body.readUtf8()).jsonObject
                            "{}"
                        }
            ...
            compose.waitUntil(15000) { compose.onAllNodesWithText("Modifier").fetchSemanticsNodes().isNotEmpty() }
