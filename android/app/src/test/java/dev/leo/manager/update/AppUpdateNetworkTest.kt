...
class AppUpdateNetworkTest {
    ...
    @Test fun checksVersionWithoutSendingSessionCredentials() = runBlocking {
        MockWebServer().use { server ->
            ...
            server.enqueue(MockResponse().setBody(Json.encodeToString(update.copy(versionCode = BuildConfig.VERSION_CODE))))
    ...
    private fun info(code: Int, signer: String = "aabb") = PackageInfo().apply {
    ...
    @Test fun refusesWrongSignerPackageVersionAndUnsupportedSdk() {
        ...
        invalid.forEach { archive ->
            assertThrows(IllegalArgumentException::class.java) { validateApkIdentity(archive, installed, update, 26) }
        }
