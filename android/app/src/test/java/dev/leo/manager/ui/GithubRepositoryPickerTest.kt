...
class GithubRepositoryPickerTest {
    ...
    @Test fun retryAutoPaginationFilterSelectionAndChange() {
        MockWebServer().use { server ->
            server.enqueue(MockResponse().setResponseCode(502).setBody("""{"error":"GitHub indisponible"}"""))
            server.enqueue(MockResponse().setBody("""{"repositories":[{"fullName":"team/existing","name":"existing","owner":"team","imported":true},{"fullName":"team/site","name":"site","language":"Vue"}],"nextPage":2}"""))
            server.enqueue(MockResponse().setBody("""{"repositories":[{"fullName":"team/selected","name":"selected","defaultBranch":"develop","private":true,"language":"Rust","stars":1200}],"nextPage":null}"""))
            server.start()
            ...
            compose.setContent {
                ...
                LeoTheme { Surface(Modifier.fillMaxSize()) { Column { GithubRepositoryPicker(api, selected) { selection = it; selected = it.fullName } } } }
            }
            ...
            compose.onNodeWithText("Rechercher un dépôt, un propriétaire, un langage").performTextInput("rust")
            ...
            compose.onNodeWithText("Rechercher un dépôt, un propriétaire, un langage").assertDoesNotExist()
    ...
    @Test fun describesRecentActivityInFrench() {
