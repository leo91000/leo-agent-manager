...
class ClaudeDeviceTest {
    ...
    @Test fun claudeLoginCompletesWithAuthorizationCode() {
        MockWebServer().use { server ->
                ...
                override fun dispatch(request: RecordedRequest): MockResponse {
                    val result = when (request.path) {
                        ...
                        "/api/claude/connection" -> {
                            ...
                            """{"connected":$connected,"maxConcurrent":$maxConcurrent,"email":"claude@example.test","login":${if (pending) """{"id":"attempt","state":"pending","url":"https://claude.ai/oauth/authorize?fixture=1"}""" else "null"}}"""
                        }
            ...
            compose.onNodeWithText("Conversations Claude simultanées").performScrollTo().performTextReplacement("2")
            ...
            compose.waitUntil(10000) { compose.onAllNodesWithText("Enregistrer la limite").fetchSemanticsNodes().isNotEmpty() }
