...
internal fun ArtifactSharingDialog(vm: LeoViewModel, artifact: Deliverable, close: () -> Unit) {
    ...
    suspend fun load(visibility: String? = null) {
        ...
        error = null
        try {
            current = if (visibility == null) vm.api.get<Deliverable>("$path?metadata=1")
            else wireJson.decodeFromString<Deliverable>(vm.api.request("PUT", "$path/visibility",
                buildJsonObject { put("visibility", visibility) }))
        } catch (e: CancellationException) { throw e }
        catch (e: Exception) { error = e.message ?: "Impossible de modifier le partage." }
        finally { busy = false }
    }
    ...
    AlertDialog(
        ...
        text = {
            Column(Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                Text(if (public) "Lien public activé" else "Fichier privé", style = MaterialTheme.typography.titleSmall)
                Text("Toute personne disposant du lien peut lire cette version sans connexion. Les autres fichiers et versions restent privés.")
                ...
                if (public) {
                    ...
                    Text("Désactiver le lien bloque les prochains accès, pas les copies déjà téléchargées.", style = MaterialTheme.typography.bodySmall)
                }
                error?.let {
                    Text(it, color = MaterialTheme.colorScheme.error)
                    TextButton(enabled = !busy, onClick = { scope.launch { load() } }) { Text("Réessayer") }
                }
                OutlinedButton(enabled = !busy && current != null,
